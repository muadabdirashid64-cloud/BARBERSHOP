...
import { LayoutDashboard, Calendar, Home, Heart, LogOut, Menu } from 'lucide-react';
...
const navItems: { to: string; label: TranslationKey; icon: typeof LayoutDashboard; color: string }[] = [
  { to: '/customer', label: 'dashboard', icon: LayoutDashboard, color: 'bg-blue-500' },
  { to: '/customer/book', label: 'bookAppointment', icon: Calendar, color: 'bg-gold-500' },
  { to: '/customer/home-services', label: 'homeServices', icon: Home, color: 'bg-pink-500' },
  { to: '/customer/favorites', label: 'favorites', icon: Heart, color: 'bg-red-500' },
];
    ...
    <div className="flex h-full flex-col bg-ink-950">
      <div className="flex items-center gap-3 px-6 py-6">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400">{t('customerPanel')}</p>
        </div>
      </div>
      ...
      <div className="border-t border-white/5 p-4">
        <div className="flex items-center gap-3 rounded-xl bg-white/5 p-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-xs text-gray-400">{t('customer')}</p>
          </div>
    ...
    <div className="flex h-screen bg-gray-50 dark:bg-ink-950">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto scrollbar-thin">
          <div className="mx-auto max-w-7xl p-6 lg:p-8"><Outlet /></div>
        </main>
      </div>
    </div>
  );
